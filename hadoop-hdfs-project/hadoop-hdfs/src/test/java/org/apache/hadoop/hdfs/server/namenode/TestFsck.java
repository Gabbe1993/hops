...
public class TestFsck {
  ...
  static final Pattern numCorruptBlocksPattern =
      Pattern.compile(".*Corrupt blocks:\t\t([0123456789]*).*");
  ...
  public void testFsckPermission() throws Exception {
    ...
    try {
      ...
      UserGroupInformation fakeUGI = UserGroupInformation
          .createUserForTesting("ProbablyNotARealUserName",
  ...
  public void testFsckMove() throws Exception {
    ...
    try {
      ...
      CorruptedTestFile ctFiles[] = new CorruptedTestFile[]{
          new CorruptedTestFile(fileNames[0], Sets.newHashSet(0), dfsClient,
              NUM_DATANODES, DFS_BLOCK_SIZE),
          new CorruptedTestFile(fileNames[1], Sets.newHashSet(2, 3), dfsClient,
              NUM_DATANODES, DFS_BLOCK_SIZE),
          new CorruptedTestFile(fileNames[2], Sets.newHashSet(4), dfsClient,
              NUM_DATANODES, DFS_BLOCK_SIZE),
      ...
      while (true) {
        ...
        if (numCorrupt == null) {
          throw new IOException(
              "failed to find number of corrupt " + "blocks in fsck output.");
        }
  ...
  static private class CorruptedTestFile {
    ...
    public void removeBlocks()
      ...
      for (int corruptIdx : blocksToCorrupt) {
        ...
        ExtendedBlock block = dfsClient.getNamenode()
            .getBlockLocations(name, blockSize * corruptIdx, Long.MAX_VALUE)
            .get(0).getBlock();
  ...
  public void testFsckMoveAndDelete() throws Exception {
    ...
    try {
      ...
      ExtendedBlock block = dfsClient.getNamenode()
          .getBlockLocations(corruptFileName, 0, Long.MAX_VALUE).get(0)
          .getBlock();
      for (int i = 0; i < 4; i++) {
        File blockFile = MiniDFSCluster.getBlockFile(i, block);
        if (blockFile != null && blockFile.exists()) {
  ...
  /**
   * Test for checking fsck command on illegal arguments should print the
   * proper
   * usage.
   */
  @Test
  ...
  public void testFsckMissingReplicas() throws IOException {
    ...
    try {
      ...
      NamenodeFsck fsck =
          new NamenodeFsck(conf, namenode, nettop, pmap, out, NUM_REPLICAS,
              (short) 1, remoteAddress);
