...
public class ImageWriter implements Closeable {
  ...
  static final int LAYOUT_VERSION = -63; // see NameNodeLayoutVersion

  final Path outdir;
  final FileSystem outfs;
  final File dirsTmp;
  final OutputStream dirs;
  final File inodesTmp;
  final OutputStream inodes;
  final MessageDigest digest;
  final FSImageCompression compress;
  ...
  long curBlock;
  AtomicLong curInode;
  final FileSummary.Builder summary = FileSummary.newBuilder()
  ...
  public ImageWriter(Options opts) throws IOException {
    ...
    if (null == opts.outStream) {
      ...
      try (NNStorage stor = new NNStorage(opts.getConf(),
        ...
        stor.format(NNStorage.newNamespaceInfo());
      }
    ...
    curBlock = startBlock = opts.startBlock;
  ...
  void writeStringTableSection() throws IOException {
    ...
    OutputStream sec = beginSection(raw);
    s.writeDelimitedTo(sec);
    for (Entry<Integer,String> e : u.entrySet()) {
  ...
  public static class Options implements Configurable {
    ...
    public void setConf(Configuration conf) {
      ...
      outdir = new Path(conf.get(OUTPUT_DIR, def));
