...
public abstract class BlockResolver {
  ...
  protected BlockInfo buildBlock(long blockId, long bytes) {
    BlockInfo bi = new BlockInfo();
    bi.setNoPersistance(blockId, bytes, 1001);
    return bi;
  }
   ...
   */
  public Iterable<BlockInfo> resolve(FileStatus s) {
    List<Long> lengths = blockLengths(s);
    ArrayList<BlockInfo> ret = new ArrayList<>(lengths.size());
    long tot = 0;
    for (long l : lengths) {
      tot += l;
